...
# Needed to build prefix in build environment
#
...
do_tree() {
	local x
	for x in etc{,/portage} usr/{{,s}bin,$(rapx "" lib)} var/tmp var/lib/portage var/log/portage var/db; do
...
bootstrap_stage1() {
	...
	*-linux*)
		if [[ ! -x "${ROOT}"/tmp/usr/bin/gcc ]] &&
			[[ $(gcc -print-prog-name=as),$(gcc -print-prog-name=ld) != /*,/* ]]; then
...
bootstrap_stage2() {
	...
	if [[ ! -x "${ROOT}"/tmp/usr/bin/makeinfo ]]; then
	...
	if [[ ${CHOST} == x86_64-*-solaris* || ${CHOST} == sparcv9-*-solaris* ]]; then
...
bootstrap_stage3() {
	...
	if [[ ! -x "${ROOT}"/usr/bin/makeinfo ]]; then
...
Normally I can only give you ~amd64 packages, and you would be exposed to all the bugs of the newest untested software.  Well, ok, sometimes
...
EOF
	while true; do
		...
		*) STAT='stat -c %U/%G' ;;
		...
		if [[ $(${STAT} "${EPREFIX}"/.canihaswrite) != $(${STAT} "${EPREFIX}") ]]; then
			echo
